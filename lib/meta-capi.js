...
/** Envoi CAPI (webhook Stripe, route navigateur). Pas d'event Purchase sur /api/capi-event. */
export async function sendCapiEvent({
  ...
  const payload = {
    ...
    event_source_url: String(pageUrl || 'https://yogyface.fr').slice(0, 500),
    user_data: hashUserData({ email, phone, firstName, fbp, fbc, ip, ua }),
    custom_data: customData,
  }
  const body = { data: [payload] }
  const test = process.env.META_TEST_EVENT_CODE
  if (test) body.test_event_code = test
  return postCapi(body)
}
...
export async function handleCapiEvent(req, res) {
  try {
    ...
    await sendCapiEvent({
      eventName,
      eventId,
      pageUrl: pageUrl || 'https://yogyface.fr/merci-liste-attente',
      email,
      phone,
      firstName,
      fbp,
      ...
      customData: { content_name: 'liste-attente' },
    })
