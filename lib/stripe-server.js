...
import { PLANS, UPSELL, UPSELL_ENABLED } from '../src/lib/stripe-offers.js'
...
/**
 * Mensualités limitées (VIP 3×, Studio 4×/6×/10×) : schedule puis stop.
...
async function ensureInstallmentSchedule(stripe, subscriptionId) {
  ...
  try {
    await stripe.subscriptionSchedules.update(schedule.id, {
      end_behavior: 'cancel',
      phases: [{ ...base, duration: { interval: 'month', interval_count: months } }],
    })
  } catch {
    // Comptes encore sur l'ancienne API (iterations).
    await stripe.subscriptionSchedules.update(schedule.id, {
      end_behavior: 'cancel',
      phases: [{ ...base, iterations: months }],
    })
  }
}
...
export async function createCheckoutSession(req, res) {
  try {
    ...
    const params = {
      ...
      success_url: `${origin}/${UPSELL_ENABLED ? 'vente-upsell' : 'merci-achat'}?session_id={CHECKOUT_SESSION_ID}`,
    ...
    if (spec.mode === 'payment') {
      ...
      params.subscription_data = {
        metadata: { ...offerMeta, max_invoices: String(spec.months || 3) },
      }
    }
