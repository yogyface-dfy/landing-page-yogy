...
/** 3 × 99,99 € : cappe l'abonnement pour éviter un 4ᵉ prélèvement. */
async function ensureThreeXCap(stripe, subscriptionId) {
  const sub = await stripe.subscriptions.retrieve(subscriptionId)
  if (sub.metadata?.max_invoices !== '3') return
  if (sub.cancel_at || sub.cancel_at_period_end) return
  const end = periodEnd(sub)
  if (!end) return
  await stripe.subscriptions.update(sub.id, { cancel_at: end + 62 * 24 * 3600 })
}
...
async function upsellCheckoutUrl(stripe, customerId, parentSessionId, origin) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: { kind: 'upsell', parent_session: parentSessionId },
    line_items: [
      {
        price_data: {
          currency: 'eur',
...
export async function createCheckoutSession(req, res) {
  try {
    ...
    const params = {
      ...
      metadata: { kind: 'vip', offer: spec.offer, plan, orig_email: trimmed },
    ...
    if (spec.mode === 'payment') {
      ...
      params.subscription_data = {
        metadata: { kind: 'vip', offer: spec.offer, plan, orig_email: trimmed, max_invoices: '3' },
      }
    }
...
export async function getCheckoutSession(req, res) {
  try {
    ...
    if (subId) await ensureThreeXCap(stripe, subId)
...
export async function chargeUpsell(req, res) {
  try {
    ...
    if (!customerId || !pm) {
      const url = await upsellCheckoutUrl(stripe, customerId, session.id, origin)
  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      if (session.mode === 'subscription' && session.subscription) {
        await ensureThreeXCap(stripe, session.subscription)
      }
    }
