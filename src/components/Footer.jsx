...
export default function Footer() {
  return (
    <footer className="bg-noir text-white py-20 px-[5%] relative overflow-hidden">
      {/* Subtle glow */}
      ...
      <div className="max-w-[1400px] mx-auto relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 mb-16">
          <div className="col-span-2 md:col-span-1">
            ...
            <div className="flex gap-3">
              <a href="https://www.instagram.com/yogyface/" target="_blank" rel="noreferrer"
                className="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center hover:border-corail hover:text-corail hover:scale-110 transition-all duration-300">
              ...
              <a href="https://www.youtube.com/@LauryYoGyFace" target="_blank" rel="noreferrer"
                className="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center hover:border-corail hover:text-corail hover:scale-110 transition-all duration-300">
                <svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                </svg>
              </a>
              <a href="mailto:contact@yogyface.fr"
                className="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center hover:border-corail hover:text-corail hover:scale-110 transition-all duration-300">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {[['/faq', 'FAQ'], ['/contact', 'Contact'], ['/vip', 'Liste d\'attente']].map(([href, label]) => (
                <li key={href}>
                  <Link to={href} className="text-white/40 text-sm hover:text-corail transition-colors duration-300">{label}</Link>
                </li>
              ))}
              <li>
                <a href="mailto:contact@yogyface.fr" className="text-white/40 text-sm hover:text-corail transition-colors duration-300">contact@yogyface.fr</a>
              </li>
            </ul>
          </div>
        ...
        <div className="border-t border-white/6 pt-8 flex flex-col sm:flex-row justify-between items-center gap-3">
          ...
          <p className="text-white/10 text-xs font-serif italic">Reprogrammation neuro-faciale par Laury · contact@yogyface.fr</p>
        </div>
      </div>
    </footer>
  )
