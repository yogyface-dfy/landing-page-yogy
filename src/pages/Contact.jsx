...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        "Une erreur est survenue. Réessaie ou écris-nous à contact@yogyface.fr",
      );
    } finally {
      setLoading(false);
    }
    ...
    <>
      ...
      <section className="relative pt-32 md:pt-40 pb-16 px-[5%]">
        <div className="max-w-[600px] mx-auto text-center">
          <div className="section-badge justify-center">Parlons-en</div>
          <h1 className="font-display text-[clamp(2.5rem,7vw,5rem)] font-black leading-[0.9] tracking-tighter text-noir mb-6">
            ÉCRIS-MOI
            <br />
            <span className="text-corail">DIRECTEMENT</span>
          </h1>
          <p className="text-gris text-[15px] md:text-[17px] leading-relaxed">
            Une question sur la méthode, une hésitation avant de te lancer, ou
            simplement envie d'échanger ? Je lis personnellement chaque message
        ...
        <div className="max-w-[600px] mx-auto">
          {sent ? (
            <div className="text-center py-16 bg-creme rounded-3xl border border-noir/5 px-6">
              ...
              <h2 className="font-display font-black text-2xl md:text-3xl tracking-tight mb-3">
                Message bien reçu !
              </h2>
              <p className="text-gris text-[15px] md:text-[16px] leading-relaxed mb-4 max-w-md mx-auto">
                Merci de m'avoir écrit. Tu vas recevoir un{" "}
                <strong className="text-noir">email de confirmation</strong> à
                l'adresse que tu as indiquée.
              </p>
              <p className="text-gris text-[15px] md:text-[16px] leading-relaxed mb-6 max-w-md mx-auto">
                Je lis personnellement chaque message et{" "}
                <strong className="text-noir">je te réponds sous 48h</strong>,
            ...
            <div className="animate-on-scroll bg-creme rounded-3xl p-5 md:p-8 border border-noir/5">
              <div className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold uppercase tracking-wider text-gris mb-2">
                      Nom
                    </label>
                    <input
                      type="text"
                      value={form.nom}
                      onChange={(e) =>
                        setForm((v) => ({ ...v, nom: e.target.value }))
                      }
                ...
                <div>
                  ...
                  <select
                    value={form.sujet}
                    onChange={(e) =>
                      setForm((v) => ({ ...v, sujet: e.target.value }))
                    }
                    className="w-full px-4 py-3 rounded-xl bg-white border border-noir/10 text-noir text-sm focus:outline-none focus:border-corail transition-colors"
                  >
                    <option>Informations sur le programme</option>
                    <option>CollaborationPartenariat</option>
                    <option>Autre</option>
                  </select>
                </div>
                ...
                <button
                  ...
                  disabled={loading}
                  className="btn-corail w-full text-base py-4 disabled:opacity-60 disabled:cursor-wait"
                >
                ...
                <p className="text-center text-gris/50 text-xs">
                  Nous te répondons sous 24h.
                </p>
              </div>
            </div>
          )}
