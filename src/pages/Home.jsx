...
const pillars = [
  { letter: 'R', title: 'Rééduquer', desc: 'Je t\'aide à reprogrammer les mauvaises habitudes posturales et musculaires qui creusent tes traits et affaissent ton visage.' },
  { letter: 'E', title: 'Équilibrer', desc: 'Je corrige l\'asymétrie de ton visage et harmonise la tonicité des deux côtés pour retrouver ton harmonie naturelle.' },
  { letter: 'S', title: 'Stimuler', desc: 'J\'active ta circulation sanguine et lymphatique pour relancer la production de collagène et retrouver l\'éclat.' },
  { letter: 'E', title: 'Éliminer', desc: 'Je libère les tensions accumulées dans ta mâchoire, ton cou et tes trapèzes qui "tirent" ton visage vers le bas.' },
  { letter: 'T', title: 'Tonifier', desc: 'Je renforce tes 57 muscles du visage pour lifter naturellement l\'ovale, les joues et les paupières — sans chirurgie.' },
]
...
const testimonials = [
  {
    ...
    name: 'Sophie M.', info: '47 ans · 8 semaines', initial: 'S'
  },
  {
    ...
    name: 'Isabelle R.', info: '44 ans · 6 semaines', initial: 'I'
  },
  {
    ...
    name: 'Christine R.', info: '51 ans · 2 mois', initial: 'C'
  },
]
  ...
  return (
    <>
      ...
      <section className="relative min-h-screen flex items-center pt-24 pb-16 px-[5%] overflow-hidden">
        ...
        <div className="absolute -top-32 -right-32 w-96 h-96 rounded-full bg-corail/10 blur-3xl pointer-events-none animate-float" data-parallax="-0.08" />
        <div className="absolute -bottom-48 -left-48 w-[600px] h-[600px] rounded-full bg-bleu/20 blur-3xl pointer-events-none animate-float-slow" data-parallax="0.05" />
        ...
        <div className="max-w-[1400px] mx-auto w-full relative z-10">
          <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center">
            <div className="order-2 md:order-1">
              <div className="animate-on-scroll section-badge" data-anim="fade" data-delay="100">
              ...
              <p className="animate-on-scroll text-[15px] md:text-[17px] text-gris leading-relaxed mb-3 md:mb-4 max-w-md" data-delay="400">
                Je suis Laury, fondatrice de YoGyFace. J'ai créé ma propre méthode de yoga du visage pour t'aider à retrouver un visage tonique, lumineux et défini — naturellement, en 10 minutes par jour.
              </p>
              <p className="animate-on-scroll text-[14px] md:text-[15px] text-gris/70 leading-relaxed mb-6 md:mb-8 max-w-md" data-delay="500">
                <span className="font-serif italic text-[15px] md:text-[16px]">Sans bistouri. Sans aiguille. Sans dépendance aux produits cosmétiques.</span>
              </p>

              <div className="animate-on-scroll grid grid-cols-2 gap-2 mb-6 md:mb-8 max-w-xs" data-delay="600">
                {features.map(f => (
                  <div key={f.label} className="flex items-center gap-2 text-xs md:text-sm text-gris group">
                    <span className="w-4 h-4 rounded-full border border-corail/30 flex items-center justify-center text-[10px] text-corail group-hover:bg-corail group-hover:text-white transition-all duration-300">✓</span>
                    {f.label}
                  </div>
                ))}
              </div>
      ...
      <section className="py-3 md:py-4 border-y border-noir/6 overflow-hidden bg-white/80 backdrop-blur-sm">
        <div className="flex animate-marquee whitespace-nowrap">
          {[...stats, ...stats, ...stats, ...stats].map((s, i) => (
            <span key={i} className="inline-flex items-center gap-3 mx-6 md:mx-10 text-xs md:text-sm font-semibold text-noir/50">
          ...
          <div className="grid md:grid-cols-2 gap-10 md:gap-20 items-center">
            ...
            <div>
              ...
              <p className="animate-on-scroll text-gris text-[15px] md:text-[16px] leading-relaxed mb-3 md:mb-4" data-delay="100">
                Le yoga du visage, c'est une gymnastique douce et ciblée des 57 muscles de ton visage. Comme n'importe quel muscle du corps, les muscles faciaux ont besoin d'être entraînés, étirés et détendus.
              </p>
              <p className="animate-on-scroll text-gris text-[15px] md:text-[16px] leading-relaxed mb-3 md:mb-4" data-delay="200">
                Avec le temps, le stress, la posture devant les écrans et le manque de stimulation, ces muscles s'affaissent. Le résultat : perte de volume, rides, relâchement, teint terne.
              </p>
              <p className="animate-on-scroll text-gris text-[15px] md:text-[16px] leading-relaxed mb-6 md:mb-8" data-delay="300">
                <strong className="text-noir">Mon approche va plus loin</strong> que le yoga du visage classique. J'ai développé ma propre méthode — <span className="font-serif italic">RESET™</span> — qui combine reprogrammation neuro-faciale, drainage lymphatique, relaxation des tensions et tonification musculaire ciblée.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {sciencePoints.map((d, i) => (
                  ...
                  >
                    ...
                    <h4 className="font-display font-black text-[14px] md:text-[15px] tracking-tight mb-1">{d.title}</h4>
                    <p className="text-gris text-xs leading-relaxed">{d.desc}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {pillars.map((p, i) => (
              ...
              >
                <div className="w-10 md:w-12 h-10 md:h-12 rounded-xl bg-corail/10 flex items-center justify-center mb-3 md:mb-4 group-hover:bg-corail group-hover:scale-110 transition-all duration-300">
                  <span className="font-display font-black text-xl md:text-2xl text-corail group-hover:text-white transition-colors duration-300">{p.letter}</span>
                </div>
                <h3 className="font-display font-black text-base md:text-lg tracking-tight mb-1 md:mb-2">{p.title}</h3>
                <p className="text-gris text-xs md:text-sm leading-relaxed">{p.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ═══ FAIT MAIN ═══ */}
      <section className="py-16 md:py-28 px-[5%] bg-white relative overflow-hidden">
        <div className="absolute top-20 right-0 w-48 h-48 opacity-[0.02] pointer-events-none hidden md:block"
          ...
          <div className="grid md:grid-cols-2 gap-8 md:gap-16 items-center">
            ...
            <div>
              ...
              <p className="animate-on-scroll text-gris text-[15px] md:text-[17px] leading-relaxed mb-4 md:mb-6" data-anim="fade" data-delay="300">
                Je ne suis pas une application. Je ne suis pas un algorithme. Chaque programme que je crée est <strong className="text-noir">100% personnalisé, à la main</strong>. J'analyse ton visage, je comprends tes besoins, et je construis ta routine sur-mesure.
              </p>
              <div className="animate-on-scroll" data-anim="fade" data-delay="400">
                <blockquote className="border-l-2 border-corail/30 pl-4 md:pl-5">
                  <p className="text-gris text-[14px] md:text-[15px] leading-relaxed font-serif italic">
                    "C'est justement parce que je fais tout manuellement que chaque programme est unique. Ça prend du temps — mais c'est ce qui fait la différence."
                  </p>
                  <p className="text-corail text-sm font-semibold mt-3">— Laury</p>
                </blockquote>
              </div>
            </div>
          </div>
            ...
            {testimonials.map((t, i) => (
              ...
              >
                <div className="flex gap-0.5 mb-3 md:mb-4">
                  {[...Array(5)].map((_, j) => (
                    <svg key={j} className="w-3.5 md:w-4 h-3.5 md:h-4 text-corail/70" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                  ))}
                </div>
                <p className="text-noir/70 text-[14px] md:text-[15px] leading-relaxed mb-4 md:mb-6 font-serif italic">"{t.text}"</p>
